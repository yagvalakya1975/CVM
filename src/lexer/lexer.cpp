...
void Lexer::scanToken() {
    switch (char c = advance()) {
        ...
        case '!': 
            ...
            else cerr << "LexerError line " << line << ": unexpected '!'.\n"; 
            break;
        ...
        case '\n': ++line; break;
        default:
            if (isDigit(c)) check_number(); 
            else if (isAlpha(c)) check_identifier();
...
void Lexer::check_identifier() 
{
    ...
    if (found != keywords.end()) addToken(found->second, text);
...
void Lexer::addToken(TokenType type, string text) { tokens.emplace_back(type, move(text), line); }
